...
model. Densa must never classify the human-readable `message` text.

`codex_error_info: "usage_limit_exceeded"` is the sole usage-limited discriminator. `reset_at` is
an optional Unix timestamp supplied by the structured event; when it is absent or invalid, Densa
