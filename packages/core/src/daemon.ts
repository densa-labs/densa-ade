...
import {
  CoreRuntimeExecutionService,
  type CoreRuntimeExecutionRequest,
} from "./core-runtime-execution.js";
...
export interface CoreDaemonOptions {
  ...
  /** Optional adapter injection for deterministic tests; production lazily uses CodexAdapter. */
...
export class CoreDaemon {
  ...
  readonly #runtimeExecution: CoreRuntimeExecutionService;
  ...
  >();
  readonly #pendingExecutions = new Map<string, CoreRuntimeExecutionRequest>();
  ...
  private constructor(
    ...
    const adapter = options.agentAdapter ?? new CodexAdapter();
    ...
    this.#runtimeExecution = new CoreRuntimeExecutionService(
      database,
      adapter,
    ...
    this.#runtimeMutations = new CoreRuntimeMutations(
      ...
      instanceId,
      adapter,
      (request) => this.#requestExecution(request),
    );
    this.#token = token;
  ...
  async #performStop(): Promise<void> {
    ...
    for (const execution of this.#activeExecutions.values()) {
      execution.controller.abort("Densa ADE Core is stopping");
    }
  ...
  #requestExecution(request: CoreRuntimeExecutionRequest): void {
    const key = String(request.projectId);
    if (this.#activeExecutions.has(key)) {
      this.#pendingExecutions.set(key, request);
      return;
    }
    ...
    const promise = Promise.resolve()
      .then(async () => {
        await this.#runtimeExecution.execute({ ...request, signal });
      })
      .catch((error: unknown) => {
        const project = this.#database.repositories.projects.findById(request.projectId);
        const reason = new SecretRedactor()
          .text(error instanceof Error ? error.message : String(error))
          .slice(0, MAX_PROTOCOL_ERROR_MESSAGE_BYTES);
        if (
          project !== undefined &&
          stateTransitionService.canTransitionProject(project.state, "BLOCKED")
        ) {
          this.#runtimeStore.transition(
            request.projectId,
            "BLOCKED",
            request.actor,
            reason || "Production execution failed",
          );
        }
        this.#runtimeStore.event(request.projectId, "RUNTIME_EXECUTION_FAILED", request.actor, {
          reason: reason || "Production execution failed",
        });
      })
      .finally(() => {
        ...
        if (pending !== undefined && this.#stopPromise === undefined) {
          this.#pendingExecutions.delete(key);
          queueMicrotask(() => this.#requestExecution(pending));
        }
      });
    this.#activeExecutions.set(key, Object.freeze({ controller, promise }));
  }
  ...
  async #dispatch(
    ...
    switch (request.method) {
      ...
      case "projects.stop": {
        ...
        if (method === "projects.resume" && result.status === "RESUMED") {
          const pending = this.#runtimeViews.approvals(controlRequest.projectId);
          this.#requestExecution({
            projectId: controlRequest.projectId,
            workspacePath: controlRequest.workspacePath,
            actor: controlRequest.actor,
            gates: {
