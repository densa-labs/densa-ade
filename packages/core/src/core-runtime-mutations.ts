...
import { CodexAdapter, type AgentAdapter } from "@densa-ade/agent-sdk";
import {
  AdaptiveInterviewPlanner,
  AgentAdapterMasterInterviewAgent,
} from "./adaptive-interview.js";
...
import { ProjectRundownService } from "./rundown.js";
...
import type { CoreRuntimeExecutionRequest } from "./core-runtime-execution.js";
...
export class CoreRuntimeMutations {
  ...
  constructor(
    ...
    private readonly requestExecution?: (request: CoreRuntimeExecutionRequest) => void,
  ) {
  ...
  private launch(
    ...
    actor: string,
    approvals: Pick<CoreRuntimeExecutionRequest, "phaseApproval" | "guidedTaskApproval"> = {},
  ): void {
    const permissionBlockers: Array<{
      readonly id: string;
      readonly scope: "project";
      readonly reason: string;
    }> = [];
    let gitAuthorization: AuthorizedOperationContext | undefined;
    let waitingForPermission = false;
    for (const operation of ["write_workspace", "git_mutation"] as const) {
      const permission = this.executionPermission(projectId, operation, actor);
      if (operation === "git_mutation" && permission.authorization !== undefined) {
        gitAuthorization = permission.authorization;
      }
      if (permission.disposition === "allow") continue;
      waitingForPermission ||= permission.disposition === "ask_user";
      permissionBlockers.push({
        id: permission.decisionId,
        scope: "project",
        reason:
          permission.disposition === "ask_user"
            ? `User approval is required for ${operation}`
            : `Current policy denies ${operation}`,
      });
    }
    if (permissionBlockers.length > 0) {
      const project = this.store.project(projectId);
      const target = waitingForPermission ? "WAITING_FOR_USER" : "BLOCKED";
      if (
        project.state !== target &&
        stateTransitionService.canTransitionProject(project.state, target)
      ) {
        this.store.transition(
          ...
          waitingForPermission
            ? "Production execution is waiting for exact permission approval"
            : "Production execution is blocked by the current permission policy",
        );
      }
      return;
    }
    ...
    if (
      ...
      stateTransitionService.canTransitionProject(project.state, "RUNNING")
    ) {
      this.store.transition(
        projectId,
        "RUNNING",
        actor,
        "Exact production execution permissions were approved",
      );
    }
    const pending = this.views.approvals(projectId);
    this.requestExecution?.({
      projectId,
      workspacePath,
      actor,
      gates: {
        outstandingUserDecisionIds: pending
          .filter((approval) => approval.kind === "roadmap_revision")
          .map((approval) => approval.proposal.proposalEventId),
        permissionBlockers: [
          ...permissionBlockers,
          ...pending
            .filter((approval) => approval.kind === "permission")
            .map((approval) => ({
      ...
      ...approvals,
      ...(gitAuthorization === undefined ? {} : { gitAuthorization }),
    });
  }

  private executionPermission(
    projectId: ProjectId,
    operation: Extract<PermissionOperation, "write_workspace" | "git_mutation">,
    actor: string,
  ): Readonly<{
    disposition: "allow" | "ask_user" | "deny";
    decisionId: string;
    authorization?: AuthorizedOperationContext;
  }> {
    const service = new PermissionPolicyService(this.db);
    const evaluated = evaluatePermissionPolicy(service.getConfiguration(projectId), operation);
    if (evaluated.disposition !== "ask_user") {
      const result = service.authorize({
        projectId,
        operation,
        actor,
        reason: "Authorize the exact production task lifecycle side effects",
    ...
    const request = this.db.eventJournal
      .replay({
        projectId,
        types: ["PERMISSION_DECISION_RECORDED"],
        limit: 1_000,
      })
      .filter(
        (event) =>
          event.payload["operation"] === operation && event.payload["disposition"] === "ask_user",
      )
      .at(-1);
    if (request !== undefined) {
      const decisionId = String(request.payload["decisionId"] ?? request.id);
      const resolution = this.db.eventJournal
        .replay({
          projectId,
        ...
        .findLast(
          (event) =>
            event.sequenceNumber > request.sequenceNumber &&
            (event.payload["decisionId"] === decisionId ||
              event.payload["requestEventId"] === request.id),
        );
      if (resolution === undefined) {
        return { disposition: "ask_user", decisionId };
      }
      if (resolution.payload["resolution"] !== "approve") {
        return { disposition: "deny", decisionId };
      }
      const approvalDecisionId = resolution.payload["approvalDecisionId"];
      const approvalCategory = resolution.payload["approvalCategory"];
      if (typeof approvalDecisionId !== "string" || typeof approvalCategory !== "string") {
        return { disposition: "deny", decisionId };
      }
      const result = service.authorize({
        projectId,
        operation,
        actor,
        reason: "Consume the exact approved production task lifecycle permission",
        occurredAt: nowIso(this.now),
        approvalDecisionId: decisionIdSchema.parse(approvalDecisionId),
        approvalCategory,
      });
      return {
        disposition: result.decision.disposition,
  ...
  private recoverSafelyInterruptedTasks(projectId: ProjectId, actor: string): boolean {
    const interrupted = this.db.repositories.tasks
      .listByProjectId(projectId)
      .filter((task) => task.state === "INTERRUPTED");
    if (interrupted.length === 0) return false;
    const recoverable = interrupted.every((task) => {
      const attempt = this.db.repositories.attempts.listByTaskId(task.id).at(-1);
      if (attempt?.completedAt === undefined) return false;
      const completion = this.db.eventJournal
        .replay({
          projectId,
          taskId: task.id,
          types: ["ATTEMPT_COMPLETED"],
          limit: 1_000,
        })
        .findLast(
          (event) =>
            event.payload["attemptId"] === attempt.id && event.payload["outcome"] === "interrupted",
        );
      if (completion === undefined) return false;
      const rollback = this.db.repositories.attemptRollbackPlans.findByAttemptId(attempt.id);
      return rollback?.appliedAt !== undefined;
    });
    ...
    this.db.transaction(() => {
      for (const task of interrupted) {
        this.db.persistStateTransition(
          stateTransitionService.transitionTask(task, "RETRYING", {
            actor,
            occurredAt,
            reason: "A new start decision resumed a safely rolled-back interrupted attempt",
          }),
      ...
      this.db.persistStateTransition(
        stateTransitionService.transitionProject(project, "RUNNING", {
          actor,
          occurredAt,
          reason: "A new start decision resumed safely interrupted project work",
        }),
        eventIdSchema.parse(`runtime-${randomUUID()}`),
      );
    });
  ...
  async createProject(input: {
    ...
    for (const existing of this.db.repositories.projects.list()) {
      let runtime;
      try {
        runtime = this.store.state(existing.id);
      } catch {
        continue;
      }
      if (runtime.workspacePath !== workspacePath) continue;
      if (
        existing.name !== name ||
        existing.executionMode !== executionMode ||
        runtime.initialIdea !== idea
      ) {
        throw new CoreRuntimeError(
          "WORKSPACE_CONFLICT",
          "The canonical workspace is already bound to a different Densa ADE project",
        );
      }
      const specification = this.db.repositories.specifications.findByProjectId(existing.id);
      if (specification === undefined) {
        throw new CoreRuntimeError(
          "PERSISTENCE_FAILURE",
          "The existing workspace project has incomplete initialization state",
        );
      }
      return {
        project: existing,
        workspacePath,
        interviewQuestions: specification.specification.unresolvedQuestions.map((question) => ({
          ...question,
    ...
    let interview;
    try {
      interview = await new AdaptiveInterviewPlanner(
        new AgentAdapterMasterInterviewAgent(this.adapter(), { cwd: workspacePath }),
      ).start(idea);
    } catch (error) {
    ...
    const specification = projectSpecificationSchema.parse(interview.specification);
    this.db.transaction(() => {
    ...
    return { project: stored, workspacePath, interviewQuestions: interview.questions };
  }
  ...
  async answerInterview(input: {
    ...
    const planner = new AdaptiveInterviewPlanner(
      new AgentAdapterMasterInterviewAgent(this.adapter(), {
        cwd: this.store.state(id).workspacePath,
      }),
    );
    const resumed = planner.resume(record.specification);
    const batch = resumed.questionBatches[0];
    if (batch === undefined) {
      throw new CoreRuntimeError(
        "USER_CONFIGURATION_ERROR",
        "Project interview has no unanswered question batch",
      );
    }
    let answered;
    try {
      answered = await planner.answerBatch({
        snapshot: resumed,
        batchId: batch.id,
        answers: input.answers.map((answer) => ({
          questionId: answer.questionId.trim(),
          answer: redactSensitiveText(answer.answer).trim(),
        })),
      });
    } catch (error) {
      throw this.asRuntimeError(error);
    ...
    const occurredAt = nowIso(this.now);
    const specification = projectSpecificationSchema.parse(answered.specification);
    this.db.repositories.specifications.set({
      projectId: id,
      specification,
    ...
    return {
      projectId: id,
      specification,
      nextQuestions: answered.questions,
      readyForRoadmap: answered.readiness.readyForRoadmap,
    };
  }
  ...
  async generateRoadmap(input: {
    ...
    if (existing === undefined) {
      let roadmap: MasterRoadmap;
      try {
        roadmap = (
          await new MasterRoadmapGenerator(
            new AgentAdapterMasterRoadmapAgent(this.adapter(), { cwd: workspacePath }),
          ).generate(specRecord.specification)
        ).roadmap;
      } catch (error) {
        throw this.asRuntimeError(error);
      }
      portable = (await service.storeInitialRoadmap(String(id), roadmap)).portableSync;
    } else {
      portable = await service.synchronizePortableProjection(String(id), {
        actor,
        reason: "Recover or verify the initial portable roadmap projection",
      });
    }
    if (portable.status !== "synchronized") {
      const runtime = this.store.state(id);
      const message =
        portable.status === "conflict"
          ? `Portable roadmap conflicts with human edits: ${portable.conflicts
              .map((conflict) => conflict.path)
              .join(", ")}`
          : portable.status === "failed"
            ? portable.message
            : "Portable roadmap projection did not synchronize";
      const code =
        portable.status === "conflict"
          ? "WORKSPACE_CONFLICT"
          : portable.status === "failed"
            ? portable.code
            : "PERSISTENCE_FAILURE";
      this.store.write(id, { ...runtime, lastError: message });
      throw new CoreRuntimeError(code, message);
    }
    ...
    if (project.state === "PLANNING") {
      ...
      portable = await service.synchronizePortableProjection(String(id), {
        actor,
        reason: "Synchronize the portable roadmap after the authoritative project became ready",
      });
      if (portable.status !== "synchronized") {
        const runtime = this.store.state(id);
        const message =
    ...
    if (runtime.lastError !== undefined) {
      const { lastError, ...recovered } = runtime;
      void lastError;
      this.store.write(id, recovered);
    }
    void sessionId;
  ...
  async startProject(input: {
    ...
    if (project.state === "RUNNING") {
      this.launch(id, workspacePath, actor);
      return { projectId: id, state: this.store.project(id).state, firstPhaseId: first.id };
    }
    if (project.state === "PLANNING") {
    ...
    } else if (project.state === "WAITING_FOR_USER") {
      this.launch(id, workspacePath, actor);
      return { projectId: id, state: this.store.project(id).state, firstPhaseId: first.id };
    } else if (project.state === "BLOCKED") {
      if (!this.recoverSafelyInterruptedTasks(id, actor)) {
  ...
  async masterSend(input: {
    ...
    const agent = new AgentAdapterMasterAgent(this.adapter(), { cwd: workspacePath });
    let proposal: ReturnType<typeof masterAgentProposalSchema.parse> | undefined;
    const service = new MasterAgentService(
      {
        propose: async (request) => {
          proposal = masterAgentProposalSchema.parse(await agent.propose(request));
          return proposal;
        },
      },
      reader,
      new ValidatedMasterCoreCommandGateway(this.db, { now: this.now }),
      new ProjectRundownService(this.db),
    );
    let handled;
    try {
    ...
    if (proposal === undefined) {
      throw new CoreRuntimeError(
        ...
        "Master Agent completed without a validated proposal",
      );
    }
    return {
      proposal,
      ...(handled.commandResult === undefined
        ? {}
        : {
            commandStatus: handled.commandResult.status,
            commandDetails: handled.commandResult.details,
          }),
    };
  }
  ...
  async approveTask(input: {
    ...
    if (input.decision === "approve") {
      ...
      this.launch(id, runtime.workspacePath, actor, { guidedTaskApproval: { taskId } });
  ...
  async approvePhase(input: {
    ...
    this.launch(id, runtime.workspacePath, actor, { phaseApproval: { phaseId } });
    return {
      projectId: id,
      phase: updated,
  ...
  async resolvePermission(input: {
    ...
    const operation =
      input.resolution === "approve"
        ? permissionOperationSchema.safeParse(request?.payload["operation"])
        : undefined;
    if (operation !== undefined && !operation.success) {
      throw new CoreRuntimeError(
        "USER_CONFIGURATION_ERROR",
        "Permission approval is missing its exact operation",
      );
    }
    const approvalCategory =
      input.resolution === "approve"
        ? `approval.permission.${operation!.data}.${canonicalDecision}`
        : undefined;
    const approvalDecisionId =
      input.resolution === "approve"
        ? decisionIdSchema.parse(sanitizeId("permission-approval"))
        : undefined;
    this.db.transaction((repositories) => {
      if (approvalDecisionId !== undefined && approvalCategory !== undefined) {
        repositories.decisions.create({
          id: approvalDecisionId,
          projectId: id,
          kind: "decision",
          statement: `User approved ${operation!.data} for permission request ${canonicalDecision}`,
          title: `Approve ${operation!.data}`,
          rationale: reason,
      ...
      repositories.events.append({
        ...
        payload: {
          ...
          ...(approvalCategory === undefined ? {} : { approvalCategory }),
        },
      });
    });
    if (input.resolution === "approve") {
      const runtime = this.store.state(id);
      if (runtime.executionRequested === true) {
        this.launch(id, runtime.workspacePath, actor);
      }
    }
    return {
      projectId: id,
