# AGENTS.md — Densa ADE Engineering Constitution

> **Scope:** These instructions apply to the entire Densa ADE repository unless a more specific nested `AGENTS.md` deliberately narrows them.
>
...
Densa ADE is a local-first, Code - OSS-based AI development IDE that turns a user's idea into a structured project roadmap, delegates implementation to coding agents, validates their work independently, persists execution state, and lets the user choose how much control to retain.
...
Densa ADE is **not** primarily an AI chat sidebar. The core product is the project lifecycle and orchestration layer.

## 1. Non-negotiable architecture

### 1.1 Densa ADE Core is authoritative

The authoritative project state lives in **Densa ADE Core**, a local process independent of the IDE UI.

The IDE, CLI, Dashboard, Roadmap, and Master Agent are clients of Densa ADE Core. They may request mutations, but they do not directly mutate authoritative state.

Never make the renderer, a webview, an extension-host object, or an in-memory React store the source of truth.

### 1.2 Densa ADE Core must remain editor-independent

Densa ADE Core must not import VS Code workbench APIs or depend on Code - OSS internals.

The architectural direction is:

```text
Densa ADE / densa-ade CLI
        |
...
A future CLI, remote UI, or another editor must be able to reuse Densa ADE Core.
...
2. a built-in Densa ADE extension;
...
Do not rewrite or fork the editor, Explorer, terminal, SCM, debugger, extension host, Monaco, or other mature upstream systems merely to make them "more Densa ADE."
...
### 1.4 Do not couple Densa ADE to unstable private VS Code agent APIs

Densa ADE may learn from VS Code's agent architecture, but v0.1 must not require private/experimental Agent Host APIs or undocumented internal protocols.

If a stable public protocol later becomes a better fit, add it behind an adapter. Do not redesign Densa ADE Core around it mid-v0.1.
...
### 3.2 `.densa-ade/` is the portable human/agent representation
...
Do not put secrets in `.densa-ade/`.

SQLite may contain richer runtime details than `.densa-ade/`, but important project intent, decisions, and phase reports must remain inspectable without opening the database.
...
For normal project creation, Densa ADE produces the full phase-level roadmap before Phase 1 begins.
...
Densa ADE must never scrape browser cookies, steal session tokens, store ChatGPT passwords, or impersonate an unsupported login flow.

For Codex, the user authenticates with the official Codex client/CLI. Densa ADE invokes the authenticated tool.
...
Only Densa ADE's validation pipeline may transition a task to `COMPLETED`.
...
Do not use broad destructive cleanup (`git reset --hard`, `git clean -fdx`, blanket file deletion) against a user's workspace unless Densa ADE can prove the content belongs to its isolated checkpoint/worktree and policy explicitly allows it.

If user-authored uncommitted changes are present, preserve them or stop and ask.

### 7.2 Densa ADE does not push in v0.1

Densa ADE may:
- create a Densa ADE working branch;
- create task commits;
- create internal refs/checkpoints.

Densa ADE does **not** automatically push to remotes.
...
```text
densa-ade: SEARCH-004 implement cursor pagination
```

A failed attempt should not be hidden. Attempt history remains in Densa ADE state even if the code is rolled back.
...
Requirements:
- prevent idle **system** sleep only while policy says Densa ADE should remain available;
- allow display sleep;
...
Densa ADE Core's local control endpoint must not be anonymously exposed.
...
Do not break standard Code - OSS interactions merely to make Densa ADE feel novel.

Densa ADE-specific surfaces should coexist with:
- Explorer;
- editor tabs;
- terminal;
- Problems;
...
v0.1 target:
- macOS;
- TypeScript/Node for Densa ADE Core;
- SQLite;
- Git;
...
- end-to-end Densa ADE project smoke tests.
...
Important decisions go into `.densa-ade/DECISIONS.md` for a Densa ADE-managed project, or the repository architecture docs for Densa ADE itself.
...
- Every future commit must use the `densa-ade:` prefix.
- Use the commit message format: `densa-ade: P<phase>M<milestone> <short description>`.
...
For development of Densa ADE itself, every successfully completed milestone MUST be pushed to the configured private `origin` repository after its milestone commit is created.

Rules:
- Push only after all milestone acceptance checks pass and the milestone commit exists.
- Push the current Densa ADE development branch to its configured upstream on `origin`.
...
- Do not automatically push user projects built *by Densa ADE*. This rule applies specifically to development of the Densa ADE codebase unless a user project explicitly opts into remote pushes.
...
- build the Code - OSS fork before the headless Densa ADE Core loop works end-to-end;
