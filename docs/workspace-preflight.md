...
`WorkspacePreflight` is the read-only Phase 3 safety boundary that runs before Densa ADE establishes a
...
cherry-pick state. It also reports ignored Densa ADE-named database, PID, socket, and
`.densa-ade/runtime/` artifacts without treating them as user changes.

The `densa-ade/run/` branch namespace is reserved for Densa ADE-owned run branches. Preflight recognizes
...
| Clean `densa-ade/run/*` branch            | `PROCEED / EXISTING_DENSA_RUN`                 |
...
Densa ADE runtime artifacts do not make an otherwise clean repository dirty.
